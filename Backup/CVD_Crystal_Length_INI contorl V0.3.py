...
def process_ini_file(config_path):
    ...
    def process_excel_file(file_path):
        ...
        if 'key_Start_Date_Time' in fields:
            ...
            # Read the last running record date
            last_run_date = read_running_rec(running_rec)

            # Filter out rows with key_Start_Date_Time older than the last running record date
            if 'key_Start_Date_Time' in fields:
                start_date_col = int(fields['key_Start_Date_Time'][0])
                df = df[df[start_date_col].apply(pd.to_datetime, errors='coerce') >= last_run_date]
            else:
...
def read_running_rec(running_rec_path):
    ...
    def update_running_rec(running_rec_path, end_date):
        try:
            ...
            # Ensure the running_rec file exists before updating it
            ensure_running_rec_exists_and_update(running_rec, datetime.today())

    # Ensure the running_rec file exists before updating it
    ensure_running_rec_exists(running_rec)
    update_running_rec(running_rec, datetime.today())
def main():
