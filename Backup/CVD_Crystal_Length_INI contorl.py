...
def process_ini_file(config_path):
    ...
    def process_excel_file(file_path):
        ...
        while row_number < row_end:
            ...
            if conn is None:
                Log.Log_Error(global_log_file, data_dict.get('key_Serial_Number', 'Unknown') + ' : ' + 'Connection with Prime Failed')
                row_number += 1
            ...
            if None in data_dict.values():
                ...
                generate_xml(data_dict, row_number, row_end)
            row_number += 1

    def generate_xml(data_dict, row_number, row_end):
        ...
        # Update .ini file with key_Start_Date_Time + key_Serial_Number
        #running_rec_file = config.get('Paths', 'running_rec')
        running_rec_file = running_rec
        if running_rec_file and row_number == row_end - 1:
                try:
                    running_rec_config = ConfigParser()
                    if os.path.exists(running_rec_file):
                        running_rec_config.read(running_rec_file, encoding='utf-8')
                    else:
                        running_rec_config.add_section('Records')
            
                    if not running_rec_config.has_section('Records'):
                        running_rec_config.add_section('Records')

                    record_key = f"{data_dict.get('key_Start_Date_Time', '')}_{data_dict.get('key_Serial_Number', '')}"
                    running_rec_config.set('Records', record_key, 'Processed')

                    with open(running_rec_file, 'w', encoding='utf-8') as configfile:
                        running_rec_config.write(configfile)
                    Log.Log_Info(global_log_file, f'Updated running_rec file with key: {record_key}')
                except Exception as e:
                    Log.Log_Error(global_log_file, f'Error updating running_rec file: {e}')
