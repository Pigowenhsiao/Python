...
def read_running_rec(running_rec_path: str) -> datetime:  # 定義 read_running_rec 函數，讀取運行記錄文件
    ...
    DayGap = 5
    if not os.path.exists(running_rec_path):  # 如果文件不存在Da
    
        with open(running_rec_path, 'w', encoding='utf-8') as f:  # 創建一個空文件
            f.write('')  # 寫入空字串
        return datetime.today() - timedelta(days=DayGap)  # 返回DayGap天前的日期
    try:  # 嘗試讀取文件內容
        ...
        return datetime.today() - timedelta(days=DayGap)  # 返回30天前的日期
...
def process_excel_file(file_path: str, sheet_name: str, data_columns, running_rec: str,
                       ...
                       operation: str, Test_Station: str) -> None:  # 定義 process_excel_file 函數，處理 Excel 文件
    ...
    try:  # 嘗試讀取 Excel 數據
        ...
        df['key_SORTNUMBER'] = df.index + 2  # 新增一欄 'key_SORTNUMBER'，值為索引加100
    ...
    # Combine the first and second columns to form a datetime column
    ...
    # Move 'Start_date_Time' to the first column and drop the original date and time columns
    cols = df.columns.tolist()
    cols.insert(0, cols.pop(cols.index('key_Start_Date_Time')))
    df = df[cols]
    df = df.drop(columns=[0, 1])
    df = df.reset_index(drop=True)  # 重設 DataFrame 的索引
    df.columns = range(df.shape[1])  # 將 DataFrame 欄位重新命名為 0, 1, 2, ... 
    df = df.dropna(subset=[0])  # 刪除第一欄為 NaN 的行
    # Rename DataFrame columns based on fields configuration
    ...
    df = df[pd.to_datetime(df['key_Start_Date_Time']) >= one_month_ago]  # 濾除 'Start_date_Time' 小於30天前的資料
    df['key_Serial_Number'] = df['Nine_Serial_Number'].apply(lambda x: str(x)[4:9])    
    df['key_Start_Date_Time'] = pd.to_datetime(df['key_Start_Date_Time'], format='%Y/%m/%d %H:%M:%S').dt.strftime('%Y-%m-%dT%H.%M.%S')
###
    Serial_Number = df['key_Serial_Number'].tolist()  # 取得所有序號    
    conn, cursor = SQL.connSQL()  # 建立資料庫連線
    if conn is None:  # 如果連線失敗
        Log.Log_Error(global_log_file, "Connection with Prime Failed")  # 記錄錯誤日誌
        return  # 結束函數執行
    try:  # 嘗試執行 SQL 查詢
        for serial in Serial_Number:  # 遍歷所有序號
            part_number, nine_serial_number = SQL.selectSQL(cursor, serial)  # 執行 SQL 查詢獲取部件號及九位序號
            if part_number and nine_serial_number:  # 如果查詢結果有效
                df.loc[df['key_Serial_Number'] == serial, 'Part_Number'] = part_number  # 更新 Part_Number 欄位
                df.loc[df['key_Serial_Number'] == serial, 'Nine_Serial_Number'] = nine_serial_number  # 更新 Nine_Serial_Number 欄位
            else:  # 如果查詢結果無效
                Log.Log_Error(global_log_file, f"Serial number {serial} not found in database")  # 記錄錯誤日誌
    except Exception as e:  # 捕捉 SQL 查詢異常
        Log.Log_Error(global_log_file, f"SQL query failed: {e}")  # 記錄錯誤日誌
    finally:  # 無論是否成功，都執行以下代碼
        SQL.disconnSQL(conn, cursor)  # 斷開資料庫連線
    ###
    #df['Part_Number'] = "TESTusePART"  # 塞入 "TESTusePART" 到 Part_Number 欄位
    #df['Nine_Serial_Number'] = "TESTUSPART2"  # 塞入 "TESTUSPART2" 到 Nine_Serial_Number 欄位
    
    
    df = df.dropna(subset=['Part_Number'])  # 刪除 Part_Number 欄位為 NaN 的行
    df = df.reset_index(drop=True)  # 重設 DataFrame 的索引
    row_end = len(df)  # 取得 DataFrame 的總行數
    row_number = 0  # 初始化行號為 0
    while row_number < row_end:  # 當行號小於總行數時，循環處理每一行
        data_dict = {}  # 初始化空字典以存儲該行數據
        if row_number == row_end - 1:  # 如果是最後一行
            latest_date = df['key_Start_Date_Time'].max()  # 取該欄的最大日期
            update_running_rec(running_rec, latest_date)  # 更新運行記錄文件 
        for key, (col, dtype) in fields.items():  # 遍歷所有配置的字段
            try:  # 嘗試轉換該字段數據
                value = df.iloc[row_number, int(col)]  # 取得指定行和列的數據
                ...
                data_dict[key] = None  # 設置該字段為 None 
                continue  # 繼續處理下一字段
        data_dict['key_SORTNUMBER'] = df.loc[row_number, 'key_SORTNUMBER']  # 將該列數存入字典
        data_dict['Part_Number'] = df.loc[row_number, 'Part_Number']
        data_dict['key_Serial_Number'] = df.loc[row_number, 'key_Serial_Number']
        data_dict['key_Operation'] = operation  # 設定操作名稱為 AFM_Step_Height
        try:  # 嘗試進行日期轉換
        ...
        Row_Number_Func.next_start_row_number(log_file, row_number)  # 更新起始行號至指定文件
...
def generate_xml(data_dict: dict, output_path: str, site: str, product_family: str,
    ...
    Log.Log_Info(global_log_file, f"XML File Path: {xml_filepath}")  # 將 XML 文件路徑寫入日誌
...
def process_ini_file(config_path: str) -> None:  # 定義 process_ini_file 函數，處理 .ini 配置文件
    ...
    global global_log_file, input_paths, output_path, xml_path, running_rec, sheet_name, data_columns, log_path, log_file, fields, site, product_family, operation, Test_Station, file_name_pattern, file_location, DayGap  # 使用全局變數
    ...
    try:  # 嘗試從配置文件中獲取各項配置
        input_paths = [path.strip() for path in config.get('Paths', 'input_paths').split(',')]  # 取得輸入路徑列表
        ...
        fields_config = config.get('DataFields', 'fields').splitlines()  # 取得數據字段配置並以行分割
    ...
    log_file = os.path.join(log_folder_path, log_file)  # 構造日誌文件完整路徑
    ...
    for input_path in input_paths:  # 遍歷所有輸入路徑
        ...
        for file in files:  # 遍歷每個匹配的文件
            if not os.path.basename(file).startswith('~$'):  # 如果文件名稱不以 '~$' 開頭
                destination_dir = file_location  # 設定目標目錄
                ...
                Log.Log_Info(global_log_file, f"Copy excel file {file} to {file_location}")  # 記錄文件複製訊息
                ...
                process_excel_file(copied_file_path, sheet_name, data_columns, running_rec,
                                   output_path, fields, site, product_family, operation, Test_Station)  # 處理該 Excel 文件
