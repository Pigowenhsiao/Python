...
def process_excel_file(file_path: str, sheet_name: str, data_columns, running_rec: str,
    ...
    try:  # 嘗試讀取 Excel 數據
        ...
        df['key_SORTNUMBER'] = df.index + 100  # 新增一欄 'key_SORTNUMBER'，值為索引加100
    ...
    if 'key_Start_Date_Time' in fields:  # 如果配置中包含 key_Start_Date_Time 欄位
        ...
        df = df[df[start_date_col].apply(pd.to_datetime, errors='coerce') >= one_month_ago]  # 過濾出日期大於等於30天前的行
    ...
    if 'key_AFM_Start_Date_Time' in fields:  # 如果配置中包含 key_AFM_Start_Date_Time 欄位
        start_AFM_date_col = int(fields['key_AFM_Start_Date_Time'][0])  # 取得該欄位的列號
        df = df[df[start_AFM_date_col].apply(pd.to_datetime, errors='coerce') >= one_month_ago]  # 過濾出日期大於等於30天前的行
        df[start_AFM_date_col] = df[start_AFM_date_col].apply(lambda x: pd.to_datetime(x).strftime('%Y-%m-%dT%H.%M.%S'))  # 格式化該欄位的日期
    else:  # 如果配置中未包含該欄位
        Log.Log_Error(global_log_file, "key_AFM_Start_Date_Time not found in fields configuration")  # 記錄錯誤日誌
    
    Serial_Number = df[int(fields['key_Serial_Number'][0])]  # 根據配置讀取序號欄位的數據
    ...
    try:  # 嘗試執行 SQL 查詢
        for serial in Serial_Number:  # 遍歷所有序號
            ...
            if part_number and nine_serial_number:  # 如果查詢結果有效
                df.loc[df[int(fields['key_Serial_Number'][0])] == serial, 'Part_Number'] = part_number  # 更新 Part_Number 欄位
                df.loc[df[int(fields['key_Serial_Number'][0])] == serial, 'Nine_Serial_Number'] = nine_serial_number  # 更新 Nine_Serial_Number 欄位
            else:  # 如果查詢結果無效
    ...
    while row_number < row_end:  # 當行號小於總行數時，循環處理每一行
        ...
        if row_number == row_end - 1:  # 如果是最後一行
            latest_date = df[start_date_col].max()  # 取該欄的最大日期
    ...
    for input_path in input_paths:  # 遍歷所有輸入路徑
        ...
        for file in files:  # 遍歷每個匹配的文件
            if not os.path.basename(file).startswith('~$'):  # 如果文件名稱不以 '~$' 開頭
                destination_dir = '../DataFile/044_/Ru_AFM/'  # 設定目標目錄
                ...
                Log.Log_Info(global_log_file, f"Copy excel file {file} to ../DataFile/044_/Ru_AFM/")  # 記錄文件複製訊息
