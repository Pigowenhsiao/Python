...
# Ensure MyModule is in the Python search patch
...
def process_excel_file(filepath_str, settings, log_file, csv_filepath):
    ...
    # If the INI file defines these keys, calculate them. Otherwise, they won't be created.
    if 'key_STARTTIME_SORTED' in settings.field_map or 'key_SORTNUMBER' in settings.field_map:
        Log.Log_Info(log_file, "Calculating STARTTIME_SORTED and SORTNUMBER as requested by INI file.")
        ...
        df.drop(columns=['date_excel_number', 'excel_row'], inplace=True, errors='ignore')
    ...
    # Conditionally add SORTED fields only if they are not already in the list
    if 'STARTTIME_SORTED' not in dynamic_column_order:
        dynamic_column_order.append('STARTTIME_SORTED')
    if 'SORTNUMBER' not in dynamic_column_order:
        dynamic_column_order.append('SORTNUMBER')
