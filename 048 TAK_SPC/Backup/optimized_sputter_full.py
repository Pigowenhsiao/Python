...
def process_excel_file(file_path: str, sheet_name: str, data_columns, running_rec: str,
    ...
    while row_number < row_end:  # 當行號小於總行數時，循環處理每一行
        ...
        if row_number == row_end - 1:  # 如果是最後一行
            latest_date = pd.to_datetime(df1['key_END_Date_Time']).max()  # 取 'key_Start_Date_Time' 欄的最大日期
        ...
        for key, (col, dtype) in fields.items():  # 遍歷所有配置的字段
            try:  # 嘗試轉換該字段數據
                ...
                continue  # 繼續處理下一字段
            Count_A = Count_A + 1 
        # Check if "key_Serial_Number" is "150" or "115"
        ...
        data_dict['key_Part_Number'] = df1.loc[row_number, 'Part_Number']  # 存入 Part_Number
        ...
        running_rec_file = config.get('Paths', 'running_rec')  # 從配置中獲取運行記錄文件路徑
        with open(running_rec_file, 'w', encoding='utf-8') as f:  # 以寫入模式開啟運行記錄文件
            f.write(str(row_number))  # 將當前行號寫入文件
